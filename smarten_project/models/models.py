...
class ResConfigSettings(models.TransientModel):
    ...
    module_hr_timesheet = fields.Boolean(default=True)
    ...
    def write(self, vals):
        ...
        for lead in self:
            if "stage_id" in vals:
                ...
                if new_stage and new_stage.is_won and not lead.project_id:

                    if not lead.project_type_ids:
                        raise UserError(_("Please select at least one Project Type."))

                    # Use first project type as base template
                    ...
                    main_project = base_type.project_template_id.copy({
                        "name": lead.partner_id.name or lead.name or _("New Project"),
                    ...
                    for extra_type in lead.project_type_ids[1:]:
                        ...
                        for stage in template.type_ids:
                            if not main_project.type_ids.filtered(lambda s: s.name == stage.name):
                                stage.copy({"project_ids": [(6, 0, [main_project.id])]})

                        # Copy tasks
                        for task in template.task_ids:
                            stage_match = main_project.type_ids.filtered(lambda s: s.name == task.stage_id.name)
                            task.copy({
                                "project_id": main_project.id,
                                "stage_id": stage_match[:1].id if stage_match else False,
                            })
